...
namespace ROI
{
    class ChampionBoidAgent : MonoBehaviour
    {
        ...
        [SerializeField] private int _SeperationGroup;
        public float TurnSpeed = 10;

        public float2 _Velocity;
        private float2 _Destination;
        [SerializeField] private bool _Pushable = true;
        ...
        public float StopingDistance
        {
            ...
            get { return _StopingDistance; }
        }

        public int FlockingGroup
        {
            get { return _FlockingGroup; }
        }

        public int SeperationGroup
        {
            get { return _SeperationGroup; }
            set { _SeperationGroup = value; }
        }
        ...
        private void Start()
        {
            TurnSpeed = _championData.moveData.baseRotationSpeed * _championData.moveData.rotationSpeedMultiplier;
            _StopingDistance = _championData.attackData.range;
            SetRadius(_championData.bodyRadius);
            Stop();
        }
        ...
        public float2 Position
        {
            get
            {
                float3 pos = _transform.position;
        ...
        public bool CanFlock(ChampionBoidAgent boidAgent)
        {
            return this != boidAgent && boidAgent.FlockingGroup == _FlockingGroup;
        }

        public bool CanSeperate(ChampionBoidAgent boidAgent)
        {
            return this != boidAgent && (boidAgent.SeperationGroup == _SeperationGroup);
        }

        public bool CanDistinguish(ChampionBoidAgent boidAgent)
        {
            return this != boidAgent && _Pushable;
        }

        public bool CanAvoid(ChampionBoidAgent boidAgent)
        {
            // Avoid only agents that are in front
            return this != boidAgent; // && IsInFront(agent);
        }

        /// <summary>
        /// True if agent is in front
        /// </summary>
        bool IsInFront(ChampionBoidAgent agent)
        {
        ...
        public float GetDistinguishWeigth(ChampionBoidAgent boidAgent)
        {
        ...
        {
            if (NavMesh.SamplePosition(_transform.position, out var hit, 2, NavMesh.AllAreas) &&
                NavMesh.SamplePosition(_transform.position + offset, out var hit2, 2, NavMesh.AllAreas) &&
                math.distance(hit.position, hit2.position) < 0.2f
               )
            {
                _transform.position = hit2.position;
            }
        }
        ...
        public void ApplyImpulse(float2 impulse)
        {
            ...
            var rotation = quaternion.LookRotation(math.normalizesafe(new float3(_Velocity.x, 0, _Velocity.y)),
                new float3(0, 1, 0));
            _transform.rotation = math.slerp(_transform.rotation, rotation, Time.deltaTime * TurnSpeed);
        }
    }
}